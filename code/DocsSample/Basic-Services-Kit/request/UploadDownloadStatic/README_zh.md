...
### 介绍
本示例使用[@ohos.request](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-basic-services-kit/js-apis-request.md)接口创建上传和下载任务，实现上传、下载功能，hfs作为服务器，实现了文件的上传和下载和任务的查询功能。本示例采用ArkTS静态模式编写，相比动态模式具有更好的性能表现。
...
```
UploadDownloadStatic
├── AppScope                                    
│   └── app.json5                               // APP信息配置文件
├── entry/src/main                              // 应用主模块
│   ├── ets
│   │   ├── entryability
│   │   │   └── EntryAbility.ets               // 应用入口
│   │   ├── components                          // 自定义组件
│   │   │   ├── CustomDataSource.ets           // 懒加载数据源
│   │   │   ├── FileBrowse.ets                 // 查看下载文件组件  
│   │   │   └── SelectFolderDialog.ets         // 文件夹选择对话框
│   │   ├── download                            // 下载任务相关
│   │   │   ├── RequestDownload.ets            // 下载任务
│   │   │   ├── SpeedLimitDownload.ets         // 限速下载
│   │   │   ├── WantAgentDownload.ets          // 通知栏下载
│   │   │   └── userFile                       // 用户目录下载
│   │   │       ├── AudioDownload.ets          // 音频下载
│   │   │       ├── DocumentDownload.ets       // 文档下载
│   │   │       └── MediaDownload.ets          // 媒体下载
│   │   ├── model                               // 数据模型
...
* 该示例分为三个模块：
  * 上传模块
    ...
    * 源码链接：[RequestUpload.ets](./entry/src/main/ets/upload/RequestUpload.ets)，[Upload.ets](./entry/src/main/ets/pages/Upload.ets)
  ...
  * 下载模块
    ...
    * 源码链接：[RequestDownload.ets](./entry/src/main/ets/download/RequestDownload.ets)，[Download.ets](./entry/src/main/ets/pages/Download.ets)，[FileUtils.ets](./entry/src/main/ets/utils/FileUtils.ets)，[FileBrowse.ets](./entry/src/main/ets/components/FileBrowse.ets)
...
2.本示例为Stage模型，采用ArkTS静态模式，支持API26版本SDK，SDK版本号(API Version 26)，镜像版本号(6.0)。
...
echo code/DocsSample/Basic-Services-Kit/request/UploadDownloadStatic/ > .git/info/sparse-checkout
