...
1. 在应用代码中开启Web调试开关，应用需要调用setWebDebuggingAccess接口，设置TCP Socket端口号并启用Web调试功能。
...
* 使用DevTools工具调试前端页面。
* 在应用代码中开启Web调试开关，应用需要调用[setWebDebuggingAccess](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis-arkweb/arkts-apis-webview-WebviewController.md#setwebdebuggingaccess)接口，设置TCP Socket端口号并启用Web调试功能。
* 无线调试直接通过接口设置端口号。
...
echo code/DocsSample/ArkWeb-Sta/ArkWebDebuggingWithDevtools > .git/info/sparse-checkout
