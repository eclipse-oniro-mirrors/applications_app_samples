...
ArkUICustomNode::ArkUICustomNode()
...
// 自定义绘制逻辑。
void ArkUICustomNode::OnDrawBehind(ArkUI_NodeCustomEvent *event)
{
...
void ArkUICustomNode::OnDrawFront(ArkUI_NodeCustomEvent *event)
{
    auto drawContext = OH_ArkUI_NodeCustomEvent_GetDrawContextInDraw(event);
    // 获取图形绘制对象。
    auto drawCanvas = reinterpret_cast<OH_Drawing_Canvas *>(OH_ArkUI_DrawContext_GetCanvas(drawContext));
