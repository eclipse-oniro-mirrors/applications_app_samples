...
void ArkUINode::SetId(const std::string &id)
{
...
void ArkUINode::ResetId() { nativeModule_->resetAttribute(handle_, NODE_ID); }

void ArkUINode::CheckErrorCode(int32_t errorCode)
{
...
void ArkUINode::SetCrossLanguage(bool isCross)
{
    // 构造结构体。
...
bool ArkUINode::GetCrossLanguage()
{
    // 构造结构体。
    auto option = OH_ArkUI_CrossLanguageOption_Create();
    // 获取组件的结构体配置。
...
ArkUI_IntOffset ArkUINode::GetLayoutPositionInWindow()
{
...
ArkUI_IntOffset ArkUINode::GetPositionWithTranslateInWindow()
{
...
// 处理通用事件。
void ArkUINode::RegisterOnClick(const std::function<void(ArkUI_NodeEvent *event)> &onClick, void* userData)
{
    onClick_ = onClick;
    // 注册点击事件。
...
void ArkUINode::RegisterOnTouch(const std::function<void(int32_t type, float x, float y)> &onTouch)
{
    onTouch_ = onTouch;
    // 注册触碰事件。
...
void ArkUINode::PostFrameCallback(void *data)
{
...
void ArkUINode::PostIdleCallback(void *data)
{
    auto context = OH_ArkUI_GetContextByNode(handle_);
...
void ArkUINode::OnRemoveAllChild()
{
...
void ArkUINode::OnInsertChildBefore(const std::shared_ptr<ArkUIBaseNode> &child,
    const std::shared_ptr<ArkUIBaseNode> &slibing)
{
    nativeModule_->insertChildBefore(handle_, child->GetHandle(), slibing->GetHandle());
}

void ArkUINode::OnInsertChildAfter(const std::shared_ptr<ArkUIBaseNode> &child,
    const std::shared_ptr<ArkUIBaseNode> &slibing)
{
