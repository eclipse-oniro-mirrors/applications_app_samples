 ...
 */
// [Start Cpp_ArkUICustomContainerNode]
// ArkUICustomContainerNode.cpp
// 自定义容器组件示例
#include "ArkUICustomContainerNode.h"

namespace NativeModule {

ArkUICustomContainerNode::ArkUICustomContainerNode()
...
void ArkUICustomContainerNode::SetPadding(int32_t padding)
{
    padding_ = padding;
...
void ArkUICustomContainerNode::OnMeasure(ArkUI_NodeCustomEvent *event)
{
    ...
    // 自定义测量为所有子节点大小加固定边距。该自定义节点最终的尺寸以此处设置的值为准。
    nativeModule_->setMeasuredSize(handle_, maxWidth + 2 * padding_, maxHeight + 2 * padding_); // padding需要乘以2。
}
...
void ArkUICustomContainerNode::OnLayout(ArkUI_NodeCustomEvent *event)
{
    ...
    for (uint32_t i = 0; i < totalSize; i++) {
        ...
        nativeModule_->layoutNode(child, (selfSize.width - childSize.width) / 2,
                                  (selfSize.height - childSize.height) / 2); // 除以2作居中。
    }
}

} // namespace NativeModule
// [End Cpp_ArkUICustomContainerNode]
