...
class ItemAdapter {
public:
    explicit ItemAdapter(std::vector<std::shared_ptr<ArkUIBaseNode>>& dataSource);
    virtual ~ItemAdapter();
    ArkUI_NodeAdapterHandle GetAdapter() const
    {
        return adapter_;
    }
    void InsertItem(int32_t index, const std::shared_ptr<ArkUIBaseNode>& value);
...
private:
    ...
    std::vector<std::shared_ptr<ArkUIBaseNode>> data_;
    ...
    std::function<void(std::shared_ptr<ArkUIBaseNode>)> removedNodeCallback_;
};

template<typename SubComponent>
ItemAdapter<SubComponent>::ItemAdapter(std::vector<std::shared_ptr<ArkUIBaseNode>>& dataSource)
...
template<typename SubComponent>
void ItemAdapter<SubComponent>::InsertItem(int32_t index, const std::shared_ptr<ArkUIBaseNode>& value)
{
...
void ItemAdapter<SubComponent>::OnItemDetached(ArkUI_NodeAdapterEvent* event)
{
    ...
    if (removedNodeCallback_) {
        removedNodeCallback_(std::make_shared<ArkUIBaseNode>(item));
    }
