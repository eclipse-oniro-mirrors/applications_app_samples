...
8. [创建懒加载布局 (LazyVGridLayout/LazyVWaterFlowLayout)](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/ui/arkts-layout-development-create-lazy-layout.md)。
...
8. 滚动 (Scroll)：包含ScrollSnap和ScrollModifier两个功能模块。ScrollSnap展示在List、Grid、WaterFlow、Scroll组件中的滚动对齐交互效果；ScrollModifier展示滚动修改器在List、Grid、WaterFlow、Scroll组件中的应用，通过修改器自定义滚动行为。
9. 懒加载布局（LazyVGridLayout/LazyVWaterFlowLayout）：在Scroll或List内使用LazyVGridLayout、LazyVWaterFlowLayout两种懒加载布局容器，配置columnsTemplate、rowsGap、columnsGap、space、alignItems等布局属性，支持多种布局容器混合组合使用，各容器使用独立LazyForEach数据源，通过onVisibleIndexesChange回调实现触底加载更多数据。
