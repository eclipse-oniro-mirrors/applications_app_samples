...
// 创建图片组件界面
// 函数1a：配置第一张图的基础属性（源、尺寸、缩放、插值、重复）
void ConfigureFirstImageBasicAttributes(ArkUI_NativeNodeAPI_1* nativeNodeApi, ArkUI_NodeHandle image1)
{
...
// 函数1a2：配置第一张图的渲染模式与同步加载
void ConfigureFirstImageRenderAttributes(ArkUI_NativeNodeAPI_1* nativeNodeApi, ArkUI_NodeHandle image1)
{
    // [Start image_render]
    // 设置渲染模式
...
// 函数1b：配置第一张图的颜色滤镜与显示方向
void ConfigureFirstImageAdvancedAttributes(ArkUI_NativeNodeAPI_1* nativeNodeApi, ArkUI_NodeHandle image1)
{
...
// 函数1c：注册并注销第一张图的事件
void RegisterAndUnregisterImage1Events(ArkUI_NativeNodeAPI_1* nativeNodeApi, ArkUI_NodeHandle image1,
...
ArkUI_NodeHandle CreateImageColumnAndFirstImage()
{
    ...
    g_image1 = image1;
    if (image1 != nullptr) {
        ConfigureFirstImageBasicAttributes(nativeNodeApi, image1);
        ConfigureFirstImageRenderAttributes(nativeNodeApi, image1);
        ConfigureFirstImageAdvancedAttributes(nativeNodeApi, image1);
        RegisterAndUnregisterImage1Events(nativeNodeApi, image1, column);
    }
    return column;
}
