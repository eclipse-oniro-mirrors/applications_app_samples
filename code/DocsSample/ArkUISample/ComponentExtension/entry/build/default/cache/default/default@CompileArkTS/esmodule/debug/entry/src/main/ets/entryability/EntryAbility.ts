...
function callBackFunc(info: uiObserver.NavDestinationSwitchInfo) {
    hilog.info(DOMAIN, 'testTag', '%{public}s', `testTag navDestinationSwitch from: ${JSON.stringify(info.from)} to: ${JSON.stringify(info.to)}`);
}
function callBackFunc2(info: uiObserver.NavDestinationSwitchInfo) {
    hilog.info(DOMAIN, 'testTag', '%{public}s', `testTag2 navDestinationSwitch from: ${JSON.stringify(info.from)} to: ${JSON.stringify(info.to)}`);
}
    ...
    onWindowStageCreate(windowStage: window.WindowStage): void {
        ...
        try {
            ...
            mainWindow.on('windowSizeChange', (data) => {
                hilog.info(DOMAIN, 'testTag', '%{public}s', 'Succeeded in enabling the listener for window size changes. Data: ' + JSON.stringify(data));
                let displayClass: display.Display | null = null;
                try {
                    displayClass = display.getDefaultDisplaySync();
                    hilog.info(DOMAIN, 'testTag', '%{public}s', 'display orientation is ' + JSON.stringify(displayClass.orientation));
                    // 获取屏幕的显示方向
