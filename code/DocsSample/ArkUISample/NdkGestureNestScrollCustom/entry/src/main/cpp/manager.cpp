...
static ArkUI_NativeGestureAPI_3 *gestureApi3 = nullptr;
...
// 并行手势事件回调
ArkUI_GestureRecognizer *ParallelGestureFunc(ArkUI_ParallelGestureEvent *eventParallel)
{
    // 获取并行内部手势事件回调中的用户自定义信息
    ArkUI_ParallelInnerGestureEvent *event = (ArkUI_ParallelInnerGestureEvent *)eventParallel;
...
void BindingGesture(napi_env env, napi_value arg, OH_NativeXComponent *component)
{
    ...
    gestureApi3->setGestureParallelTo(scroll, manager->GetCustomData(), ParallelGestureFunc);
}
...
napi_value Manager::CreateNativeNode(napi_env env, napi_callback_info info)
{
    ...
    OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_GESTURE, ArkUI_NativeGestureAPI_3, gestureApi3);
