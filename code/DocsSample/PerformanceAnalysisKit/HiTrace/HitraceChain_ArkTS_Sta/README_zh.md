...
```
$ hitrace -t 10 app | grep myTestAsyncTrace
```

2. 点击设备上的"testHiTraceIdPassedAutomatically"按钮（需在10秒内完成，否则步骤3捕获不到trace数据），触发HiTraceId自动传递的业务逻辑。 使用关键字"testTag"过滤日志，查看该业务代码打印的hilog日志。
```
xx-xx xx:xx:xx.xxx   xxxxx-xxxxx   A00000/com.exa...ation/testTag  com.examp...lication  I     [xxxxxxxxxxxxxxx 0 0]HiTraceId is invalid, begin hiTraceChain
xx-xx xx:xx:xx.xxx   xxxxx-xxxxx   A00000/com.exa...ation/testTag  com.examp...lication  I     [xxxxxxxxxxxxxxx 0 0]HiTraceFlag INCLUDE_ASYNC is enabled
xx-xx xx:xx:xx.xxx   xxxxx-xxxxx   A00000/com.exa...ation/testTag  com.examp...lication  I     [xxxxxxxxxxxxxxx 0 0]start testHiTraceIdPassedAutomatically async trace
xx-xx xx:xx:xx.xxx   xxxxx-xxxxx   A00000/com.exa...ation/testTag  com.examp...lication  I     [xxxxxxxxxxxxxxx 0 0]end testHiTraceIdPassedAutomatically async trace
```

3. 点击设备上的"testHiTraceIdPassedManually"按钮，触发HiTraceId手动传递的业务逻辑。

4. 在DevEco Studio Terminal窗口查看trace数据，HiTraceChain跟踪开启期间，HiTraceMeter打点得到的trace日志会自动携带HiTraceId信息。
```
e.myapplication-xxxxx   (  xxxxx) [010] .... xxxxxx.xxxxxxx: tracing_mark_write: S|xxxxx|H:[xxxxxxxxxxxxxxx,0,0]#myTestAsyncTrace|1001|M62|categoryTest|key=value
```
