...
// C：内层三方异步任务。
static void InnerAsyncTask(AsyncTaskCtx *ctx)
{
    if (ctx == nullptr) {
        return;
    }
    // 步骤2：异步任务执行时，将异步上下文压入当前线程运行上下文，建立更深一级异步调用链。
    OH_HiDebug_PushAsyncContext(ctx->asyncCtx);
    OH_LOG_INFO(LogType::LOG_APP, "[Async-C] Inner async task start, push context %{public}llu",
        (unsigned long long)ctx->asyncCtx);
    std::this_thread::sleep_for(std::chrono::milliseconds(ASYNC_TASK_DURATION_MS)); // 模拟异步耗时
    OH_LOG_INFO(LogType::LOG_APP, "[Async-C] Inner async task done");
...
static void ThirdPartyAsyncTask(AsyncTaskCtx *ctx)
{
    ...
    // 三方异步回调中提交内层异步任务C，并透传异步上下文句柄。
    OH_LOG_INFO(LogType::LOG_APP, "[Async-B] Submit inner async task C");
    auto *innerCtx = new (std::nothrow) AsyncTaskCtx{ctx->asyncCtx};
    if (innerCtx == nullptr) {
        OH_HiDebug_PopAsyncContext(ctx->asyncCtx);
        delete ctx;
        return;
    }
    std::thread inner(InnerAsyncTask, innerCtx);
    inner.join(); // 等待C完成，保证调用链时序

    // 步骤3：异步任务完成时，将异步上下文弹出，解除异步调用链。
    OH_HiDebug_PopAsyncContext(ctx->asyncCtx);
    OH_LOG_INFO(LogType::LOG_APP, "[Async-B] Third-party async task done, pop context");
    delete ctx;
}
...
static void OuterTaskFunc()
{
    ...
    worker.join(); // 等待B（及其提交的C）完成，保证Release在所有Push/Pop之后
...
// 构造A->B->C最小三方异步调用链，演示管理异步上下文的四个接口。
