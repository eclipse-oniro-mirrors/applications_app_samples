...
void SampleBitMap::DrawSimpleText() {
    // [Start ndk_drawing_simple_text_create_typographyStyle]
    // 创建一个 TypographyStyle 创建 Typography 时需要使用
    OH_Drawing_TypographyStyle *typoStyle = OH_Drawing_CreateTypographyStyle();
    // 设置文本对齐方式为居中
    OH_Drawing_SetTypographyTextAlign(typoStyle, TEXT_ALIGN_CENTER);
    // [End ndk_drawing_simple_text_create_typographyStyle]

    // [Start ndk_drawing_simple_text_create_textStyle]
    ...
    // [End ndk_drawing_simple_text_create_textStyle]

    // [Start ndk_drawing_simple_text_create_typography]
    ...
    // [End ndk_drawing_simple_text_create_typography]

    // [Start ndk_drawing_simple_text_layout_and_paint]
    // 设置页面最大宽度
    double maxWidth = width_;
    OH_Drawing_TypographyLayout(typography, maxWidth);
    // 将文本绘制到画布上
    OH_Drawing_TypographyPaint(typography, cCanvas_, 0, 100);
    // [End ndk_drawing_simple_text_layout_and_paint]

    // [Start ndk_drawing_simple_text_destroy]
    // 释放内存
    ...
    // [End ndk_drawing_simple_text_destroy]
}

napi_value SampleBitMap::NapiDrawSimpleText(napi_env env, napi_callback_info info)
{
    DRAWING_LOGI("NapiDrawSimpleText");
    if ((env == nullptr) || (info == nullptr)) {
        DRAWING_LOGE("NapiDrawSimpleText: env or info is null");
        return nullptr;
    }

    napi_value thisArg;
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        DRAWING_LOGE("NapiDrawSimpleText: napi_get_cb_info fail");
        return nullptr;
    }

    napi_value exportInstance;
    if (napi_get_named_property(env, thisArg, OH_NATIVE_XCOMPONENT_OBJ, &exportInstance) != napi_ok) {
        DRAWING_LOGE("NapiDrawSimpleText: napi_get_named_property fail");
        return nullptr;
    }

    OH_NativeXComponent *nativeXComponent = nullptr;
    if (napi_unwrap(env, exportInstance, reinterpret_cast<void **>(&nativeXComponent)) != napi_ok) {
        DRAWING_LOGE("NapiDrawSimpleText: napi_unwrap fail");
        return nullptr;
    }
    ...
    if (OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize) != OH_NATIVEXCOMPONENT_RESULT_SUCCESS) {
        DRAWING_LOGE("NapiDrawSimpleText: Unable to get XComponent id");
        return nullptr;
    }
    ...
    if (render != nullptr) {
        ...
        render->DisPlay();
        render->Destroy();
        DRAWING_LOGI("DrawSimpleText executed");
    } else {
...
void SampleBitMap::Export(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor desc[] = {
        {"drawSimpleText", nullptr, SampleBitMap::NapiDrawSimpleText, nullptr, nullptr, nullptr, napi_default, nullptr}};
