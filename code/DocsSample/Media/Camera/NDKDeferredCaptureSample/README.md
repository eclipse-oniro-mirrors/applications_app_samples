# NDKDeferredCaptureSample

### 介绍

本示例主要展示了相机的相关功能，使用[libohcamera.so]
接口实现相机的预览、分段式拍照、动态照片、前后置摄像头切换进行拍照，以及闪光灯、变焦、对焦、曝光等控制类功能。
...
10. 进入预览界面，预览正常，点击动态照片按钮，点击拍照按钮，拍照正常，左下角生成缩略图，点击左下角缩略图，能够跳转图库，图片保存正常且为动态照片。
11. 进入预览界面，预览正常，进入拍照模式，点击拍照按钮，拍照正常，左下角生成缩略图，点击左下角缩略图，能够跳转到图库，图片保存正常，打开图片显示正常。
12. 进入预览界面，预览正常，切换到前置摄像头，点击拍照按钮，拍照正常，左下角生成缩略图，点击左下角缩略图，能够跳转到图库，图片保存正常，打开图片显示正常。
...
|       |-- MovingPhotoPage.ets             // 动态照片按钮UI布局
...
* 相机功能接口实现在CameraManager.cpp中，源码参考：[CameraManager.cpp](entry/src/main/cpp/CameraManager.cpp)
    ...
    * 动态照片相关接口封装在EnableMovingPhoto接口中，主要包含以下流程：调用OH_PhotoOutput_IsMovingPhotoSupported判断是否支持动态照片，调用OH_PhotoOutput_EnableMovingPhoto使能动态照片。
...
* 相机预览、分段式拍照功能、前后置切换、动态照片功能实现调用侧位于Index.ets，modeSwitchPage.ets，main.cpp中，源码参考：[Index.ets](entry/src/main/cpp/ets/pages/Index.ets)，[modeSwitchPage.ets](entry/src/main/cpp/ets/views/modeSwitchPage.ets)，[main.cpp](entry/src/main/cpp/main.cpp)
    ...
    * 拍照：开启拍照位于ModeSwitchPage.ets下的cameraDemo.startPhotoOrVideo接口，将modelBagCol的值，videoId，拍照的surfaceID或者录像的surfaceId以及deviceDegree传入接口startPhotoOrVideo。跳转到main.cpp中的StartPhotoOrVideo接口，将传下来的参数进行格式转换，再调用CameraManager对象下的TakePicture接口开启拍照操作。
    ...
    * 动态照片：开启动态照片位于MovingPhotoPage.ets，其中调用cameraDemo.enableMovingPhoto接口，实际调用到main.cpp下的EnableMovingPhoto接口，最终调用到CameraManager.cpp下的EnableMovingPhoto接口，实现动态照片的使能。
