...
- 使用流程：
  ...
  - 录制流处于运行状态时，点击'设置静音提示'按钮，调用`audioCapturer.setMuteHint(true)`告知音频系统当前录音流已由应用业务侧静音或期望按静音状态处理，示例会自行将录制数据写入为静音数据；点击'解除静音提示'按钮，调用`audioCapturer.setMuteHint(false)`解除提示。该接口从API version 24开始支持，不会实际触发录音流静音。
