...
// [StartExclude create_renderer]
// [Start create_capturer]
static OH_AudioCapturer *g_audioCapturerVoIP;
// [StartExclude create_capturer]
// [Start activa_audiosession]
static OH_AudioSessionManager *g_audioSessionManager;
// [StartExclude activa_audiosession]
...
// [EndExclude create_renderer]
// [EndExclude create_capturer]
...
{
    // 将待播放的数据，按bufferLen长度写入buffer。
    // 如果开发者不希望播放某段buffer，返回AUDIO_DATA_CALLBACK_RESULT_INVALID即可。
    // [StartExclude create_renderer]
...
static napi_value AudioRendererPlayerInitVoIP(napi_env env, napi_callback_info info)
{
    ...
    // 当前实现接口streamUsage是外部传参，voip通话传入参数是STREAM_USAGE_VOICE_COMMUNICATION或STREAM_USAGE_VIDEO_COMMUNICATION。
...
// [EndExclude activa_audiosession]
