...
- 点击'设置录音流静音提示'按钮，即可创建并启动一条录音流，调用`OH_AudioCapturer_SetMuteHint`上报和解除录音流静音提示。该接口不会实际触发静音，仅告知音频系统该录音流已由应用静音或期望按静音状态处理。
...
- 点击'启动音频内录'按钮，即可通过`OH_AudioStreamBuilder_SetPlaybackCaptureMode`配置内录采集模式，并通过`OH_AudioCapturer_RequestPlaybackCaptureStart`请求启动音频内录；可通过'查询音频内录状态'和'停止音频内录'查看或释放内录流。
...
- 使用流程：
  ...
  - 点击'设置录音流静音提示'按钮，先创建并启动录音流，再调用`OH_AudioCapturer_SetMuteHint(audioCapturer, true)`告知音频系统当前录音流已由应用业务侧静音或期望按静音状态处理，随后调用`OH_AudioCapturer_SetMuteHint(audioCapturer, false)`解除提示。该接口从API version 24开始支持，不会实际触发录音流静音。
  ...
  - 点击'启动音频内录'按钮，首先调用`OH_AudioStreamBuilder_Create`创建音频流构造器，接着设置采样率、声道数、采样格式、编码格式和读数据回调，然后调用`OH_AudioStreamBuilder_SetPlaybackCaptureMode`配置`MEDIA | EXCLUDING_SELF`采集模式。构造录制流后调用`OH_AudioCapturer_RequestPlaybackCaptureStart`异步请求启动音频内录，并通过回调返回`SUCCESS`、`FAILED`或`NOT_AUTHORIZED`状态。
...
音频内录由`OH_AudioCapturer_RequestPlaybackCaptureStart`触发系统侧授权校验，授权结果通过异步回调返回。
