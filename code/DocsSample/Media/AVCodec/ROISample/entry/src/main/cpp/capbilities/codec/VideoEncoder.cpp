...
int32_t VideoEncoder::Configure(const SampleInfo &sampleInfo)
{
    ...
    if (sampleInfo.videoInfo.bitrateMode == BITRATE_MODE_SQR) {
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_SQR_FACTOR, 25);
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_MAX_BITRATE, 30 * 1024 * 1024);
    } else if (sampleInfo.videoInfo.bitrateMode == BITRATE_MODE_CQ) {
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_QUALITY, 100);
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_VIDEO_ENCODER_QP_MAX, 4);
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_VIDEO_ENCODER_QP_MIN, 0);
    } else {
