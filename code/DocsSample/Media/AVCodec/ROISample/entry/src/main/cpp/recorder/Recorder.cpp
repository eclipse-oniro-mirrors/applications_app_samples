...
int32_t Recorder::Init(SampleInfo &sampleInfo)
{
    ...
    if (!sampleInfo_.videoInfo.isHDRVivid) {
        int32_t ret = InitRenderThread(roiPathType);
        CHECK_AND_RETURN_RET_LOG(ret == SAMPLE_ERR_OK, ret, "Init render thread failed");
    }

    int32_t ret = InitEncoderChain(sampleInfo_, roiPathType);
    CHECK_AND_RETURN_RET_LOG(ret == SAMPLE_ERR_OK, ret, "Init encoder chain failed");
...
int32_t Recorder::InitRenderThread(RoiPathType roiPathType)
{
...
int32_t Recorder::InitEncoderChain(SampleInfo &sampleInfo, RoiPathType roiPathType)
{
...
void Recorder::AudioEncInputThread()
{
    while (true) {
        ...
        MixMicAndBgm(reinterpret_cast<int16_t *>(inputBufferAddr), audioEncContext_->decodedBgmQueue,
                     sampleInfo_.audioInfo.audioMaxInputSize / AUDIO_BYTES_PER_SAMPLE);
        lock.unlock();
