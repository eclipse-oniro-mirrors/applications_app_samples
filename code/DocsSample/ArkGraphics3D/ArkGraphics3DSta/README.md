...
本示例通过 ArkGraphics3DSta 的接口，利用轻量级的 3D 引擎和渲染管线，实现了 3D 场景创建、3D 场景资源创建和 3D 动画创建。

本示例主要展现了如何利用 ArkGraphics3DSta 来实现 3D 图像的渲染，其中利用 Scene、Camera 和 Light 来创建基础的场景，使用 Material、Shader、Image 和Environment 来实现场景资源，采用 Animation 的各种方法来播放 3D 动画，并展示了通用材质属性（透明混合、剔除模式、渲染排序、阴影接收）和 PBR 材质属性（基础色、清漆、自发光、法线、环境光遮蔽、光泽、高光等）的详细用法。
