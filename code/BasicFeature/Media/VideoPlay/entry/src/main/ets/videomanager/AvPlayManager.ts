...
export default class AvPlayManage {
  ...
  private mgr: resourceManager.ResourceManager;
  private currentTime: number; // 视频当前时间
  private durationTime: number; // 视频总长
  private speedSelect: number; // 倍速选择
  ...
  async initPlayer(surfaceId: string, callback: (avPlayer: media.AVPlayer) => void): Promise<void> {
    ...
    try {
      ...
      this.mgr = globalThis.context.resourceManager;
