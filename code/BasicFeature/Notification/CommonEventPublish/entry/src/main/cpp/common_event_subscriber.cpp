...
CommonEvent_SubscribeInfo* CreateSubscribeInfo(const char* events[], int32_t eventsNum,
                                               const char* permission, const char* bundleName)
{
    int32_t ret = -1;
    // 创建订阅者信息
...
void OnReceive(const CommonEvent_RcvData *data)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 1,
                 "CES_TEST", "event: %{public}s, code: %{public}d, data: %{public}s, bundle: %{public}s",
...
void GetParameters(const CommonEvent_RcvData *data)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 1, "CES_TEST", "exists = %{public}d, doubleValue = %{public}f",
                 exists, doubleValue);
    ...
    if (intArraySize <= 0 || *arr == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 1,
                     "CES_TEST", "exists = %{public}d, Failed to get int array or invalid size: %{public}d",
                     exists, intArraySize);
    } else {
        OH_LOG_Print(LOG_APP, LOG_INFO, 1,
                     "CES_TEST", "exists = %{public}d, intArraySize = %{public}d", exists, intArraySize);
    ...
    if (longArraySize <= 0 || *longArray == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 1,
                     "CES_TEST", "exists = %{public}d, Failed to get long array or invalid size: %{public}d",
    ...
    if (charArraySize <= 0 || *charArray == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 1,
                     "CES_TEST", "exists = %{public}d, Failed to get charArray or invalid size: %{public}d",
                     exists, charArraySize);
    } else {
    ...
    if (boolArraySize <= 0 || *boolArray == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, 1, "CES_TEST",
                     "exists = %{public}d, Failed to boolArray or invalid size: %{public}d",
                     exists, boolArraySize);
    } else {
        OH_LOG_Print(LOG_APP, LOG_INFO, 1, "CES_TEST", "exists = %{public}d, boolArraySize = %{public}d",
                     exists, boolArraySize);
...
void AbortCommonEvent(CommonEvent_Subscriber* subscriber)
{
    ...
    if (OH_CommonEvent_AbortCommonEvent(subscriber)) {
        if (OH_CommonEvent_FinishCommonEvent(subscriber)) {
            // 获取当前有序公共事件是否处于中止状态
            OH_LOG_Print(LOG_APP, LOG_INFO, 1,
                         "CES_TEST", "Abort common event success, Get abort <%{public}d>.",
