...
10. 点击心形图标，取消收藏
11. 图片详情界面，重命名文件（可重名）
12. 图片详情界面，删除文件（移动到最近删除相册）
13. 从最近删除恢复照片
14. 从最近删除永久删除照片
15. 点击视频，查看视频并播放
16. 点击图片页面右上角叹号，查看图片详情
17. 长按图片批量操作：多选/全选、删除、恢复
18. 相册内图片列表界面点击“+”，从已有相册添加照片(不能从系统相册添加）
19. 长按图片列表图片，更多菜单，从图片选择添加到已有相册（不能添加到系统相册）
20. 在相册图片列表界面，长按图片，选择图片，点击更多，可以将图片加入到其他相册（可多选）
21. 在用户相册图片列表界面，长按图片，选择图片，点击更多，可以将图片从当前相册移除（可多选）
22. 在用户相册图片列表界面，长按图片，选择图片，点击更多，可以查看图片详情（可多选）
...
2. 本示例仅支持API10版本SDK，版本号：4.0.9.2，镜像版本号：OpenHarmony 4.0.9.2，需要手动替换Full SDK才能编译通过，具体操作可参考[替换指南](https://docs.openharmony.cn/pages/v3.2/zh-cn/application-dev/quick-start/full-sdk-switch-guide.md/)；
