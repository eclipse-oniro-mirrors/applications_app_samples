...
typedef void (*CallbackStatusJSFunc)(std::string, int); // define func
...
class NDKCamera {
  public:
    ~NDKCamera();
    NDKCamera(char *str, uint32_t focusMode, std::string& cameraDeviceID, bool isVideo);
    ...
    Camera_ErrorCode CreatePreviewOutput(bool isVideo);
    ...
    Camera_ErrorCode setPhotoSettings(Camera_PhotoCaptureSetting setting);
    ...
    Camera_ErrorCode VideoOutputStart(bool enableMirror);
    ...
    Camera_ErrorCode UpdateSurfaceId(char *newSurfaceId);
    ...
    void RegisterJSCallback(CallbackStatusJSFunc callback);
  ...
  private:
    ...
    friend class CameraSetting; // 添加这行代码
    static NDKCamera *ndkCamera_;
    static std::mutex mtx_;
    volatile bool valid_;
    std::string m_strCameraID;

    Camera_ErrorCode ConfigureVideoPreviewProfiles();
    Camera_ErrorCode ConfigurePhotoPreviewProfiles();
    Camera_ErrorCode CreatePreviewOutputInner();
};
} // namespace OHOS_CAMERA_SAMPLE
