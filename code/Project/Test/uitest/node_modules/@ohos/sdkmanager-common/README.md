...
```ts
const prjSdkConfig = OhPrjSdkConfig.builder('sdk根路径').nodeHome('node路径').npmConfig('.npmrc路径').sdkProxy('代理信息');
...
// 从本地所有的OpenHarmony SDK中查找api8的toolchains
const toolchains = localSdks.get(new PathAndApiVersion(ComponentPath.TOOLCHAINS, 8));
// 获取api8的OpenHarmony toolchains，如果本地不存在该SDK，则会触发下载
