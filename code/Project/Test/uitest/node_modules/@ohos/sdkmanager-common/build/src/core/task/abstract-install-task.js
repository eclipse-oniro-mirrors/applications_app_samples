...
class AbstractInstallTask extends abstract_component_task_1.AbstractComponentTask {
    constructor(component, sdkSettings, netClient) {
        super(component);
        this._cleanLegacyFile = (filePath) => {
            try {
                fs_1.default.rmSync(filePath, { force: true, recursive: true }); // 下载前先删除之前可能残留的文件
            }
            catch (e) {
    ...
    downloadHelper(downloadUrl, downloadLocation, progress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this._cleanLegacyFile(downloadLocation);
            const isDownloadSuccess = yield this.netClient.doGetDownload(downloadUrl, downloadLocation, progress);
            if (!isDownloadSuccess) {
                throw new sdk_exception_1.SdkException('Failed to download sdk', new error_tip_1.ErrorTip('', 'The network or server is abnormal', `${node_os_1.default.EOL}1. Check the network connection and download the file again.\
            ${node_os_1.default.EOL}2. If the issue persists, choose Help > Show Log in Explorer to obtain logs, \
          and then choose Help > Contact Support for technical assistance.`));
            }
        });
